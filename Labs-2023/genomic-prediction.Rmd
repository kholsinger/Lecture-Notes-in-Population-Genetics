...
Again, the model that doesn't include population effects performs slightly better,^[It's elpd is 0.2 units smaller.] but the magnitude of the difference is less than twice the standard error, so we don't have a good reason to prefer one to the other. We'll proceed without including population in the predictions. Feel free to run through the examples below with population included to see if it makes a difference.
...
The dashed line is the 1:1 line for a perfect fit. The predictions look fairly reasonable.^[There is one outlier, which is precisely what we'd expect from the `loo()` analysis.]
