...
## Trying a larger population size

The formula used for $N_e$ to plot the expectations depends on approximations that ignore "small" terms. Increasing the population size should improve the approximation. Let's see what happens when we increase the population sizes by a factor of 100 (besides making the simulation take a lot longer).

```{r}
p_mean <- numeric(1000)
p_var <- numeric(1000)
for (i in 1:1000) {
  p <- simulate(n_f = 1000, n_m = 2000, p_0 = 0.5, n_samp = 100, 
                n_pop = 100)
  p_mean[i] <- mean(p)
  p_var[i] <- var(p)
}
...
Wow! That was pretty awful. Let's see what happens if we use an exact calculation for $N_e$ instead of throwing away small terms.
...
n_e_exact <- function(n_f, n_m) {
  n <- n_f + n_m
  f <- (1/2)*((n-1)/(2*n-1))*((1/(2*n_f)) + (1/(2*n_m)))
  n_e <- 1/(2*f)
  return(n_e)
}
...
p <- ggplot(p_dat, aes(x = p_var)) +
  ...
  geom_vline(xintercept = p_0*(1-p_0)/(2*n_e), linetype = "dashed",
             color = "salmon") +
  geom_vline(xintercept = p_0*(1-p_0)/(2*n_e_exact(10, 20)), linetype = "dashed",
             color = "blue") +
  ggtitle("Distribution of variance in allele frequencies") +
  theme_bw()
p
```
Nope! That's not it either. Let's look at the relatiionship between $p$ and Var($p$).

```{r}
p <- ggplot(p_dat, aes(x = p_mean, y = p_var)) +
  geom_point() +
  theme_bw() +
  xlab("Mean allele frequency in offspring") +
  ylab("Variance in allele frequency among offspring")
p
```
There doesn't seem to be much relationship, but let's see what happens if we compare $N_e$ calculated from the realized $p$ instead of the expected $p$.

```{r}
p_0 <- mean(p_dat$p_mean)
p_test <- tibble(var_exp = p_dat$p_mean*(1.0 - p_dat$p_mean)/(2.0*n_e),
                 var_obs = p_dat$p_var)
p <- ggplot(p_test, aes(x = var_exp, y = var_obs)) +
  geom_point() +
  xlab("Expected") +
  ylab("Observed") +
  geom_hline(yintercept = p_0*(1-p_0)/(2*n_e), linetype = "dashed",
             color = "salmon") +
  theme_bw()
p
```
...
p <- ggplot(p_dat, aes(x = p_mean)) +
  ...
  geom_vline(xintercept = 0.5, linetype = "dashed", 
             color = "salmon") +
  stat_function(fun = dnorm, args = list(mean = mean(p_dat$p_mean), sd = sd(p_dat$p_mean))) +
  ggtitle("Distribution of mean allele frequencies") +
  theme_bw()
p
p <- ggplot(p_dat, aes(x = p_var)) +
  ...
  geom_vline(xintercept = p_0*(1-p_0)/(2*n_e_exact(10, 20)), linetype = "dashed",
             color = "blue") +
  stat_function(fun = dnorm, args = list(mean = mean(p_dat$p_var), sd = sd(p_dat$p_var))) +
  ggtitle("Distribution of variance in allele frequencies") +
  theme_bw()
p
p <- ggplot(p_dat, aes(x = log(p_var))) +
  geom_histogram(bins = 25) +
  geom_vline(xintercept = log(p_0*(1-p_0)/(2*n_e)), linetype = "dashed",
             color = "salmon") +
  geom_vline(xintercept = log(p_0*(1-p_0)/(2*n_e_exact(10, 20))), linetype = "dashed",
             color = "blue") +
  stat_function(fun = dnorm, args = list(mean = mean(log(p_dat$p_var)), sd = sd(log(p_dat$p_var)))) +
  ggtitle("Distribution of variance in allele frequencies") +
  theme_bw()
p
```
