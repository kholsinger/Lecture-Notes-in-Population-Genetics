...
I use `set.seed(1234)` to ensure that when you run this example you get the same numbers I do.
...
analyze_trait <- function(dat, A, trait, n_loci = NA, n_samples = 2000, n_chains = 4) {
  if (is.na(n_loci)) {
    n_loci <- ncol(dat) - 5
  }
  ...
  for (i in 1:n_loci) {
    locus <- colnames(dat)[i + 5]
    cat("Checking locus ", i, " (", locus, ")\n", sep = "")
...
## This example analyzes only the first 10 loci
##
results <- analyze_trait(dat, A, "Mass", 10)
