...
each of those, but we're going to focus in detail only on viability
selection. Although I will say only a little about other forms of
selection, it's important to remember that any or all of the other
...
because there's a tendency to think that viability selection is the
only kind of natural selection there is.
...
\end{equation}
Suppose that instead of the fitnesses being $w_{11}$, $w_{12}$, and
$w_{22}$ they were 1, $w_{12}/w_{11}$, and $w_{22}/w_{11}$. We'd then
have the following equation:
\begin{equation}
p' = \frac{p^2 + (w_{12}/w_{11})pq}{(\bar w/w_{11})} \quad . \label{eq:relative}
\end{equation}
If you stare at that a bit, you'll realize that
equation~(\ref{eq:relative}) is equivalent to
equation~(\ref{eq:absolute}), i.e., the way in which allele
frequencies change from one generation to the next is identical in the
two cases.\footnote{If you don't see why that's true, it might help to
  realize that you could get equation~(\ref{eq:relative}) from
  equation~(\ref{eq:absolute}) by dividing the numerator and
  denominator in equation~(\ref{eq:absolute}) by $w_{11}$, i.e.,
  multiplying equation~(\ref{eq:absolute}) by 1.} I won't write out
all of the equations, but all these sets of fitnesses are equivalent
to one another:\footnote{And there are many more sets fitnesses that
...
viabilities.\footnote{We'll also see when we get to studying the
  interaction between natural selection and drift that the principle
  no longer holds, which is why I put the caveat about an infinite
  population in there. To understand how drift and selection interact,
  we have to know something about {\it absolute\/} viabilities, not
  just relative
...
all of that algebra is that it allows  us to make predictions about
...
the favored allele. With {\it disruptive selection\/} (panel (c) $w_{11} >
w_{12}$ and $w_{22} > w_{12}$) viability selection may lead either to
an increasing frequency of the $A_1$ allele or to a decreasing frequency.
  Which homozygous genotype comes to
  predominate depends on the initial allele frequency in
  the population. With {\it stabilizing selection\/} (panel (d) $w_{11} < w_{12}
  > w_{22}$; also called balancing selection or heterozygote
  advantage) viability selection leas to a stable polymorphism.}\label{fig:wbar}
\end{figure}
...
starts with an allele on the left side of the bowl $A_1$, will be lost. If
it starts on the right side of the bowl, $A_2$ will be
...
It is useful to describe patterns of fertility selection in terms of a
fitness matrix. Describing the matrix is easy. Writing it down gets
messy. Each element in the table is simply the average number of
offspring produced by a given mated pair. We write down the table with
paternal genotypes in columns and maternal genotypes in rows:
\begin{center}
\begin{tabular}{c|ccc}
\hline\hline
