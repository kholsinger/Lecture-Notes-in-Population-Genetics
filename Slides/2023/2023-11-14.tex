...
\myslide{
  \heading{Covariance between random variables}
\[
...
\myslide{
  ...
  \vfill
  TAMO\footnote{It's not a huge miracle. The details are in the notes.}
  \vfil
\[
...
\myslide{
  \heading{Regression design}

  Regress the offspring phenotype on the mean of the parental
  phenotype. The covariance between the mid-parent value and the
  offspring value is $\Cov_{PO}$.
  \vfil
  \begin{eqnarray*}
...
\myslide{
  \heading{Regression design}
  \begin{eqnarray*}
    b_{MP  \rightarrow O}  &=& \frac{\frac{1}{2}V_a}{\frac{1}{2}V_p} \\
                        &=& h^2_N
  \end{eqnarray*}
...
\myslide{
  \heading{Half-sib full-sib design}
\begin{center}
\begin{tabular}{l|ccc}
\hline\hline
        &      &             & Composition of \\
