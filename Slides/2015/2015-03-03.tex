...
\]
Expected time to coalescent event
\[
2N_e\mbox{ generations}
\]
}
...
\left(\frac{1}{2N_e}\right)\left(\frac{m(m-1)}{2}\right)
\]
Expected time to first coalescent event
\[
\bar t_m = \frac{4N_e}{m(m-1)}
\]
}

\myslide{
Expected time to common ancestry of {\it all\/} alleles in the sample
\begin{eqnarray*}
\bar t &=& \sum_k \bar t_k \\
&=& \sum_m \frac{4N_e}{m(m-1)} \\
&=& \mbox{TAMO} \\
&=& 4N_e\left(1 - \frac{1}{m}\right)
\end{eqnarray*}
\vfill
Time to common ancestry of all alleles approximately $4N_e$ generations
}
...
\vfill
\[
F_{ST} \approx \frac{\bar t - \bar t_0}{\bar t}
\]
\vfill
Another way to think about $F_{ST}$: The additional time to common
ancestry of alleles arising because populations are subdivided.
