...
The formula for $F_{ST}$ under the finite island model is a lot more complicated than the one above. If you're interested, you can look in the `R` code below.^[You won't find the formula for $F_{ST}$ in Latter, but the formulas in the `R` code follow from the results there.] Let's compare the predictions for a few values of $m$ and $k$ with $N_e = 100$ and $\mu = 10^{-3}$.
...
You can plot the observed $F_{ST}$ versus the $F_{ST}$ predicted from either model using the code below.^[The diagonal red dashed line is the 1:1 line. Every point should fall on this line if observed exactly matches the prediction.] In addition to showing how well the simulations match the predictions, you can get a feel for how much variabiity there is among the simulations by seeing how scatterd the points are.
