...
sg_opts <- paste("-q -mHKY -t0.5 -fe -l1 -s", N_e_mu, sep = "")
genotypes <- matrix(nrow = 2*n_indivs, ncol = n_loci*2)
allele_ct <- 0
tic()
for (i in 1:n_loci) {
  for (k in 1:2) {
    allele_ct <- allele_ct + 1
    ms_out <- island(2*sample_sizes, ne_m = N_e_m)
    ms_seq <- seqgen(opts = sg_opts, newick.tree = ms_out)
    for (j in 2:(2*n_indivs+1)) {
      tmp <- strsplit(ms_seq[j], "\\s+")
      genotypes[j-1, allele_ct] <- tmp[[1]][2]
    }
  }
}
genotypes <- data.frame(genotypes)
## the "each = 2" argument to rep causes each element in the preceding vector to be repeated
## twice
##
genotypes$label <- paste("Individual", rep(1:n_indivs, each = 2), sep = "")
genotypes$pop <- rep(pop_labels, each = 2)
...
genos <- read.structure("genotypes.stru", n.ind = n_indivs, n.loc = n_loci, onerowperind = FALSE, col.lab = 1,
