...
venusta <- subset(greenhouse, species == "venusta") %>%
  filter(!is.na(lwr))
venusta$mom_id <- factor(venusta$mom_id)
ven_sum <- venusta %>%
  group_by(mom_id) %>%
  summarize(mom_mean = mean(lwr), within_sd = sd(lwr))
stan_data <- list(n_indiv = nrow(venusta),
                  n_sires = length(unique(venusta$mom_id)),
                  mu_prior = mean(venusta$lwr),
                  prior_within = 1/mean(ven_sum$within_sd),
                  prior_among = 1/sd(ven_sum$mom_mean),
                  sire = as.numeric(venusta$mom_id),
                  pheno = venusta$lwr)
fit <- stan("heritability.stan",
            data = stan_data,
            refresh = 0)
