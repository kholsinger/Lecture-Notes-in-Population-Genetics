...
population.} Suppose that there are initially two alleles in this
...
\item Each offspring is an identical copy of its parent, i.e., $A_1$
  begets $A_1$ and $A_2$ begets $A_2$. In other words, there's no
  mutation.
...
\item Each gamete is an identical copy of its parent, i.e., $A_1$
begets $A_1$ and $A_2$ begets $A_2$.
...
\quad ,
\]
i.e., a binomial distribution.\index{genetic drift!binomial distribution} I'll be astonished if any of what I'm about to say is
apparent to any of you, but this equation implies three really
important things. We've encountered two already:\index{genetic drift!properties}
...
particularly important for you to remember:\index{genetic drift!properties}
...
females are different, every new offspring has exactly one father and
one mother. Thus, the probability that the first gamete selected at
random is female is just 1/2, and the probability that the second
...
\item Finally, the probability that one particular female gamete was
chosen is $1/2N_f$, where $N_f$ is the number of females in the
population. Similarly the probability that one particular male gamete
was chosen is $1/2N_m$, where $N_m$ is the number of males in the
population.
